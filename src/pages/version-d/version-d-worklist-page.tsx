...
import { BOARD_COLUMN_ORDER, fullName, movePatientToStatus, type BoardColumnId } from "@/pages/version-d/version-d-shared";
import { VersionDPatientCard, VersionDPatientCardOverlay } from "@/pages/version-d/version-d-patient-card";
import { VersionDAddPatientModal } from "@/pages/version-d/version-d-add-patient-modal";
import { VersionDSinglePatientMessageModal } from "@/pages/version-d/version-d-single-patient-message-modal";
import { VersionDStaffCancelPatientModal } from "@/pages/version-d/version-d-staff-cancel-patient-modal";
import { VersionDWorklistBulkMessageModal } from "@/pages/version-d/version-d-worklist-bulk-message-modal";
import { VersionDWorklistEmptyState } from "@/pages/version-d/version-d-worklist-empty-state";
...
export function VersionDWorklistPage() {
    const { strings } = useVEDLocale();
    const wl = strings.versionD.worklist;
    const vdShared = strings.versionD.shared;
    const {
        query,
        setQuery,
        patients,
    ...
    return (
        <main className="flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            {patients.length === 0 ? (
                <VersionDWorklistEmptyState onAddPatient={() => setAddPatientOpen(true)} />
            ) : (
                ...
                >
                    <LayoutGroup id="ved-worklist-board-version-d">
                        <div className="grid w-max min-w-[1152px] max-w-[1188px] grid-cols-[repeat(3,minmax(0,381.667px))] gap-4">
                            {BOARD_COLUMN_ORDER.map((colId: BoardColumnId) => {
                                ...
                                const colTitle = vdShared.boardColumnTitle[colId];
                                return (
                                                            ...
                                                            onMoveTo={(next) => {
                                                                ...
                                                                appendActivityLog({
                                                                    ...
                                                                    detail: wl.statusMenuDetail(
                                                                        vdShared.patientStatus[p.status],
                                                                        vdShared.patientStatus[next],
                                                                    ),
                                                                });
                    ...
                    <DragOverlay adjustScale={false} dropAnimation={cardDropAnimation} zIndex={100}>
                        {activePatient ? (
                            <div className="w-[min(100vw-2rem,381.667px)] rotate-1">
                                <VersionDPatientCardOverlay patient={activePatient} title={fullName(activePatient)} />
                            </div>
                        ) : null}
                    </DragOverlay>
                </DndContext>
            ...
            <VersionDAddPatientModal />
            <VersionDWorklistBulkMessageModal isOpen={bulkMessageOpen} onOpenChange={setBulkMessageOpen} />
            <VersionDStaffCancelPatientModal patientId={cancelModalPatientId} onDismiss={() => setCancelModalPatientId(null)} />
            <VersionDSinglePatientMessageModal patientId={singleMessagePatientId} onDismiss={() => setSingleMessagePatientId(null)} />
        </main>
    );
}
