...
export function VersionDPatientConsentPage() {
    ...
    const consentC = strings.versionD.pages.consentC;
    const consentD = strings.versionD.pages.consentD;
    const nextS = strings.versionD.pages.consentNext;
    ...
    return (
        <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto bg-[#F9FAFB]">
                    ...
                    <article className="overflow-hidden rounded-2xl border border-[#E4E6EA] bg-white shadow-[0px_4px_24px_rgba(16,24,40,0.06)]">
                        ...
                        <div className="px-6 py-6 sm:px-8 sm:py-8">
                            <p className="text-[15px] leading-relaxed text-[#172B4D]">
                                {consentC.helloPrefix} <strong>{fullName(waitingQueuePatientFromLink)}</strong>,
                            </p>
                            {waitingQueuePatientFromLink.consentManagedManually ? (
                                <>
                                    <p className="mt-4 text-[15px] leading-relaxed text-[#172B4D]">{consentD.waitingQueueManualP1}</p>
                                    <p className="mt-4 text-sm text-[#5E6C84]">{consentD.waitingQueueManualP2}</p>
                                </>
                            ) : (
                                <>
                                    <p className="mt-4 text-[15px] leading-relaxed text-[#172B4D]">{consentD.waitingQueueNonManualP1}</p>
                                    <p className="mt-4 text-sm text-[#5E6C84]">{consentD.waitingQueueNonManualP2}</p>
                                </>
                            )}
                            <div className="mt-8 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                >
                                    {waitingQueuePatientFromLink.consentManagedManually
                                        ? consentC.cancelAppointmentButton
                                        : consentC.withdrawConsentButton}
                                </Button>
                            </div>
                        </div>
                                ...
                                <>
                                    <div className="space-y-4 border-b border-[#EEF0F4] pb-6">
                                        ...
                                        <div className="space-y-4">
                                            {consentD.bodyParagraphs.map((paragraph, i) => (
                                                <p key={i} className="text-[15px] leading-relaxed text-[#172B4D]">
                                                    {paragraph}
                                                </p>
                                            ))}
                                        </div>
                                    ...
                                    <div className="mt-8 border-t border-[#EEF0F4] pt-6">
                                        ...
                                        <p className="mt-4 text-sm text-[#5E6C84]">
                                            ...
                                            <span className="font-medium text-[#172B4D]">
                                                {formatExpiryDate(selected.createdAt, locale, nextS.dateFallback7d)}
                                            </span>
                                    ...
                                    <div className="mt-8 flex flex-col gap-4">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                                            ...
                                            <Button
                                                ...
                                                onClick={() => setExitModal({ variant: "cancel_before_consent", patientId: selected.id })}
                                            >
                                                {consentD.refuseButton}
                                            </Button>
                                        </div>
            ...
            <ExitDistanceServiceConfirmModal
                ...
                variant={exitModal?.variant ?? "withdraw"}
                onConfirm={() => {
                    if (!exitModal) return;
                    if (exitModal.variant === "cancel_before_consent") refuseConsent(exitModal.patientId);
                    else if (exitModal.variant === "withdraw") withdrawConsent(exitModal.patientId);
                    else if (exitModal.variant === "cancel_manual_waiting") cancelManualWaitingEnrollmentFromPatient(exitModal.patientId);
                    setExitModal(null);
