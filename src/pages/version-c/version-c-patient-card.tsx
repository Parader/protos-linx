...
import type { BoardColumnId, Patient, PatientCompletionCause, PatientStatus } from "@/pages/version-c/version-c-shared";
import { fullName, statusToColumn, statusWhenDroppedOnColumn } from "@/pages/version-c/version-c-shared";
...
export function VersionCPatientCard({ patient, onSelect, onMoveTo }: Props) {
    const { strings } = useVEDLocale();
    const pc = strings.versionC.pages.patientCardC;
    const dash = strings.versionC.shared.dash;
    const {
    ...
    const moveToColumn = (column: BoardColumnId) => {
        const next = statusWhenDroppedOnColumn(column, patient.status);
        if (next === patient.status) return;
        onMoveTo(next);
        if (next === "recall") notifyForStatusChange(patient.id, "recall");
    };
    ...
    const onMenuAction = (key: string) => {
        switch (key) {
            case "edit":
                ...
                break;
            case "message":
                setSingleMessagePatientId(patient.id);
                break;
            case "cancel":
    ...
    return (
        <motion.div
            ref={setNodeRef}
            layoutId={`ved-patient-version-c-${patient.id}`}
            layout={!isDragging}
            initial={false}
        ...
        >
            ...
            >
                                    ...
                                    <Dropdown.Popover className="w-[min(100vw-2rem,280px)]" offset={4}>
                                        <Dropdown.Menu selectionMode="none" onAction={(k) => onMenuAction(String(k))}>
                                            <Dropdown.Section>
                                                ...
                                                <Dropdown.Item id="message" label={pc.sendMessage} icon={Mail01} selectionIndicator="none" />
                                            </Dropdown.Section>
                                            <Dropdown.Separator />
                                            <Dropdown.Section>
                                                ...
                                                <Dropdown.Item
                                                    id="move-waiting"
                                                    label={pc.colWaiting}
                                                    isDisabled={currentColumn === "waiting"}
                                                    selectionIndicator="none"
                                                />
                                                <Dropdown.Item
                                                    id="move-recall"
                                                    label={pc.colRecall}
                                                    isDisabled={currentColumn === "recall"}
                                                    selectionIndicator="none"
                                                />
                                                <Dropdown.Item
                                                    id="move-completed"
                ...
                <div className="flex items-center gap-3 pb-3">
                    <div className="flex items-center gap-2 text-sm text-[#1D2939]">
                        <Folder className="size-4 text-[#667085]" strokeWidth={1.75} aria-hidden />
                        <span className="font-medium">{patient.fileNumber}</span>
                    </div>
...
export function VersionCPatientCardOverlay({ patient, title }: { patient: Patient; title: string }) {
    const { strings } = useVEDLocale();
    const dash = strings.versionC.shared.dash;
    return (
